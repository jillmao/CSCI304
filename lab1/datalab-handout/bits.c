 ...
 * Use &0x000000ff to extract the needed bytes at the position.
 */
...
int fitsBits(int x, int n) {
     ...
     *The only binary operator left is +, so to use this, one case has to always equal 0, and the other can equal any number.
...
int isGreater(int x, int y) {
  /* Variables msbx and mxby are made to extract the signs of x and y.
   * if_same sign is made to find if x and y have the same signs or not; it will return 1 if the same. 
   * If x and y are not the same signs, then whichever one is positive is greater; 
   * this is the first half of the or statement.
   * For the second half, if x and y are the same sign, then x+(-y) will return negative.
   * To find if it is negative, it is shifted 31 bits and &1 is used.
   */
  int msbx = (x>>31)&1;
  int msby = (y>>31)&1;
  int if_same_sign = !(msbx^msby);
    //return 1 if same, return 0 if different
  int if_not_same_sign = !if_same_sign;
  int x_plus_negative_y = (y + (~x + 1));

  return ((if_not_same_sign)&(msby&1))|((if_same_sign) & (x_plus_negative_y>>31)&1);
...
int multFiveEighths(int x) {
    /*To multiply by 5: shift 2 to the left to double x, then add one more x to get 5x.
     *To divide by 8: shift right to the 3. Because 2^3 = 8 and shifting right will divide.
     *To round towards 0 for negative numbers, add (2^3)-1 or 7 before dividing by eight.
     *Because 7 is represented in 3 bits, it will be shifted away when the number is shifted during division. 
     *But it may increment the bit at the farthest right by 1 after the shift so that negative fractions can round to 0.
     */
    int multiply_five = (x<<2) + x;
    
    int for_rounding = (multiply_five >> 31) & 7;
    //for_rounding will equal 0 if multiply_five is positive.
    
    int divide_eight = ((for_rounding)+multiply_five) >>3;
 ...
 */
int sm2tc(int x) {
    /*Use (((x>>31)&1) <<31 to extract the sign bit at the correct position.
     * Use x & ~0 to extract the magnitude, and use | with the sign. This will satisfy most conditions.
     * To account for zero, bang the sign twice, this will result in 1 if -0. 
     * Then add that to sign. Only -0 will be affected.
     */
    int sign = (((x>>31)&1) <<31);
    int magnitude = x & ~0;
    int if_neg_zero = !!(sign);
    //get 1 if -0

    return (sign+(~if_neg_zero + 1))|magnitude;
}
/* 
...
unsigned float_i2f(int x) {
    /*First find the sign and shift that to the correct location.
     * To find the exponent, use a for loop to keep multiplying 1 by 2 until the number<= x.
     * Use a counter to keep track, this will be exponent.
     */
    int sign = ((x>>31)&1)<<31;
    ...
    int real_exponent = (exponent & ~0)<<27;
    
    int man = x / (2^exponent);
    int mantissa = man * 10*exponent;
        
  return sign | real_exponent | mantissa;
}
